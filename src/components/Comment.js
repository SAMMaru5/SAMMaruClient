...
function Comment(props) {
    ...
    useEffect(() => {
        api.get("/api/boards/" + props.boardId + "/articles/" + props.articleId + "/comments", "").then((response) => {
            ...
            if (document.getElementById("click0")) {
                document.getElementById("click0").className = document.getElementById("click0").className + " bg-primary text-white rounded"
            }
        })
    ...
    const insertComment = () => {
        api.post("/api/boards/" + props.boardId + "/articles/" + props.articleId + "/comments", { "content": content })
            .then((response) => {
    ...
    const change_page = (event, value) => {
        ...
        document.getElementById("click" + ((value - 1) % 5)).className = "page-link bg-primary text-white rounded"
        setCurNode(value - 1)
        if (value > all_page) {
            setComments([])
        }
    ...
    const clickRight = (event) => {
        ...
        if (cur_node + 1 < all_page) {
            document.getElementById("click" + ((cur_node) % 5)).className = "page-link"
            document.getElementById("click" + ((cur_node + 1) % 5)).className = "page-link bg-primary text-white rounded"
        ...
        <div id="comment">
            <div className='comment-frame'>
                ...
                <div className="textarea-frame">
                    <textarea value={content} onChange={(e) => { setContent(e.target.value) }} placeholder="댓글을 입력해주세요."></textarea>
                ...
                <nav aria-label="Page navigation example">
                    <ul className="d-flex justify-content-center align-items-center pagination">
                        <li className="page-item">
                            <a className="page-link" href="#!" aria-label="PreviouNexs" id="pageLeft" onClick={(e) => { clickLeft(e) }}>
                        ...
                        </li>
                        {
                            page.map((value, index) => (
                                <li className="page-item" key={index}>
                                    <a className="page-link" id={"click" + ((value - 1) % 5)} href="#!" onClick={(e) => { change_page(e, value) }}>{value}</a>
                                </li>

                            ))
                        }
                        <li className="page-item">
                            <a className="page-link" href="#!" aria-label="Next" id="pageRight" onClick={(e) => { clickRight(e) }}>
